...
def add_courses():
    ...
    if request.method == 'POST':
        ...
        course = Course(course_number=course_number, title=title, descriptions=descriptions, professor=professor)
...
def edit_course(id):
    ...
    if request.method == 'POST':
        ...
        course.descriptions = request.form['descriptions']
        professor_name = request.form['professor']
        professor = Professor.query.filter_by(name=professor_name).first()
...
def get_course_id(id):
    # return "This course's course_number is " + str(id)
